...
impl Collection {
    pub fn get(&'_ self, name: &str) -> &'_ [SetCode] {
...
pub async fn del_from_collection(card: String, to_del: SetCode) -> anyhow::Result<()> {
...
pub async fn add_to_collection(card: String, new_version: SetCode) -> anyhow::Result<()> {
    let Collection(mut collection) = load().await?;
    collection
        .entry(card)
